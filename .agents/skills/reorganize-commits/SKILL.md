...
1. **변경 사항 분석**: 대상 브랜치(기본: `yskim-hori/dev`)와 비교하여 변경된 파일들을 논리적 그룹으로 분류합니다.
...
4. **커밋별 자동 검증 (선택 사항)**:
   ...
   ```bash
   git rebase -i yskim-hori/dev --exec "./.agents/skills/reorganize-commits/scripts/verify_commit.sh"
   ```
